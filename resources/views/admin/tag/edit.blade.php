...
<div class="content">
    <div class="card border-0 rounded-3 shadow-sm mb-4">
        <div class="card-header bg-theme-1 rounded-3 shadow border-0 d-flex justify-content-between align-items-center">
            <h5 class="my-2">Edit Tag</h5>
        </div>
        <div class="card-body">
            <form method="post" action="{{ route('admin.artikel.tag.update') }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                <input type="hidden" name="id" value="{{ $tag->id_tag }}">
                <div class="form-group row">
                    <label class="col-md-2 col-form-label">Tag <span class="text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" name="tag" class="form-control {{ $errors->has('tag') ? 'border-danger' : '' }}" value="{{ $tag->tag }}">
                        @if($errors->has('tag'))
