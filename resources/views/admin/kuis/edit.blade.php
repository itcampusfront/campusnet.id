    ...
    <body>
            ...
            <div class="row">
                <div class="col-lg-8 mx-auto">
                    <form id="form-quiz" method="post" action="#">
                        {{ csrf_field() }}
                        <input type="hidden" name="id" value="{{ $kuis->id_kuis }}">
                        <!-- Judul dan Deskripsi Soal -->
                        <div class="tile" id="tile-title">
                            <div class="tile-body">
                                <div class="row">
                                    <div class="form-group col-md-12">
                                        <input type="text" name="judul_soal" class="form-control form-field" value="{{ $kuis->judul_kuis }}" placeholder="Judul Soal">
                                    </div>
                                    <div class="form-group col-md-12">
                                        <textarea name="deskripsi_soal" class="form-control form-field" placeholder="Deskripsi Soal" rows="1">{{ $kuis->deskripsi_kuis }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tile-questions">
                            ...
                            <div class="tile tile-question" data-question="{{ $i }}">
                                <div class="tile-body">
                                    <div class="row">
                                        <div class="form-row col-md-12 form-question">
                                            <div class="col">
                                                <textarea name="pertanyaan[{{ $i }}]" class="form-control form-field pertanyaan" placeholder="Pertanyaan" rows="1">{{ $kuis->pertanyaan[$i] }}</textarea>
                                                <img class="img-thumbnail mt-2 {{ $kuis->gambar_pertanyaan[$i] != '' ? '' : 'd-none' }}" src="{{ $kuis->gambar_pertanyaan[$i] }}" style="width: {{ ($kuis->dimensi_pertanyaan[$i][1] + 10) }}px">
                                                <div><a href="#" class="btn-remove-image-question $kuis->gambar_pertanyaan[$i] != '' ? '' : 'd-none'" data-question="{{ $i }}" data-choice="0">Hapus Gambar</a></div>
                                            </div>
                                            <div class="col-auto"><button class="btn btn-primary btn-add-image-question" data-question="{{ $i }}" data-choice="0" data-toggle="tooltip" data-placement="top" title="Tambahkan Gambar"><i class="fa fa-image"></i></button></div>
                                        </div>
                                        @for($j = 0; $j < count($kuis->pilihan[$i]); $j++)
                                        <div class="form-row col-md-12 form-choice" data-choice="{{ ($j+1) }}">
                                            <div class="col-auto">
                                                <input type="radio" name="kunci[{{ $i }}]" class="radio-disabled" data-toggle="tooltip" data-placement="top" title="Pilih salah satu sebagai kunci jawaban" {{ $kuis->kunci_jawaban[$i] == $j ? 'checked' : '' }}>
                                            </div>
                                            <div class="col">
                                                <input type="text" name="pilihan[{{ $i }}][{{ $j }}]" class="form-control form-field pilihan" value="{{ $kuis->pilihan[$i][$j] }}" placeholder="Pilihan">
                                                <img class="img-thumbnail mt-2 {{ $kuis->gambar_pilihan[$i][$j] != '' ? '' : 'd-none' }}" src="{{ $kuis->gambar_pilihan[$i][$j] }}" style="width: {{ ($kuis->dimensi_pilihan[$i][$j][1] + 10) }}px">
                                                <div>
                                                    <a href="#" class="btn-remove-image-choice {{ $kuis->gambar_pilihan[$i][$j] != '' ? '' : 'd-none' }}" data-question="{{ $i }}" data-choice="{{ ($j+1) }}">Hapus Gambar</a>
                                                </div>
                                            </div>
                                            <div class="col-auto">
                                                <button class="btn btn-primary btn-add-image-choice" data-choice="{{ ($j+1) }}" data-toggle="tooltip" data-placement="top" title="Tambahkan Gambar"><i class="fa fa-image"></i></button>
                                            </div>
                                            <div class="col-auto">
                                                <button class="btn btn-primary btn-remove-choice" data-choice="{{ ($j+1) }}" data-toggle="tooltip" data-placement="top" title="Hapus Pilihan" {{ count($kuis->pilihan[$i]) > 1 ? '' : 'disabled' }}><i class="fa fa-times"></i></button>
                                            </div>
                                        </div>
                                        @endfor
                                        <div class="form-row col-md-12 form-add-choice">
                                            <div class="col-auto"><input type="radio" class="radio-disabled" disabled></div>
                                            <div class="col"><a href="#" class="btn-add-choice">Tambahkan Pilihan Lainnya</a></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="tile-footer">
                                    <div class="form-row">
                                        <div class="col"></div>
                                        <div class="col-auto"><button class="btn btn-primary btn-add-question" data-toggle="tooltip" data-placement="top" title="Tambah Pertanyan"><i class="fa fa-plus"></i></button></div>
                                        <div class="col-auto"><button class="btn btn-primary btn-copy-question" data-toggle="tooltip" data-placement="top" title="Salin Pertanyan"><i class="fa fa-copy"></i></button></div>
                                        <div class="col-auto"><button class="btn btn-primary btn-remove-question" data-toggle="tooltip" data-placement="top" title="Hapus Pertanyan"><i class="fa fa-trash"></i></button></div>
                                    </div>
                                </div>
                            </div>
                            @endfor
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" id="embed-url" class="form-control" value="{{ isset($_GET) && isset($_GET['saved']) ? url('quiz/embed/'.$kuis->kode_kuis) : '' }}" readonly>
        ...
        <script type="text/javascript">
            ...
            $(window).on("load", function(){
                @if(isset($_GET) && isset($_GET['saved'])) $("#modal-embed").modal("show"); @endif
                autosize($("textarea"));
                update_question();
                load_resizable_image();
            });
        ...
        <script type="text/javascript">
            ...
            function load_resizable_image(){
                var images = $(".tile-question img");
