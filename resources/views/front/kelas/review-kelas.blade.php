    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                @endif
                <div class="card">
                    <div class="card-header">Penilaian Kelas</div>
                    <div class="card-body">
                        @if(Session::get('message'))
                        <div class="alert alert-success text-center">{{ Session::get('message') }}</div>
                        @endif
                        <form method="post" action="/kelas/review-kelas">
                            {{ csrf_field() }}
                            <input type="hidden" name="id_kelas" value="{{ $kelas->id_kelas }}">
                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label">Rating <span class="text-danger">*</span></label>
                                <div class="col-sm-9">
                                    <select name="rating" class="form-control {{ $errors->has('rating') ? 'border-danger' : '' }}" {{ check_penilaian_kelas(Auth::user()->id_user, $kelas->id_kelas) > 0 ? 'disabled' : '' }}>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <textarea name="ulasan" class="form-control {{ $errors->has('ulasan') ? 'border-danger' : '' }}" rows="3" placeholder="Masukkan Ulasan" {{ check_penilaian_kelas(Auth::user()->id_user, $kelas->id_kelas) > 0 ? 'disabled' : '' }}>{{ $penilaian ? $penilaian->review : '' }}</textarea>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <button type="submit" class="btn btn-primary" {{ check_penilaian_kelas(Auth::user()->id_user, $kelas->id_kelas) > 0 ? 'disabled' : '' }}><i class="fa fa-save mr-2"></i>Submit</button>
                                </div>
                            </div>
                        </form>
                    </div>
